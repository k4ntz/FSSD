...
parser.add_argument('-ac', '--activation', default='onesin', help='activation (default: onesin)',
                    choices=['relu', 'rat', 'rare', 'orare', 'tent', 'onesin'])
...
net = ResNet34(args.activation) # Used in FSSD paper
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, f'./checkpoint/ckpt_{args.activation}.pth')
        best_acc = acc
